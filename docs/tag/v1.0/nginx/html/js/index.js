...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadRagOptions = () => {
        ...
        fetch('http://101.126.92.213:8090/api/v1/rag/query_rag_tag_list')
...
function appendMessage(content, isAssistant = false, saveToStorage = true) {
 ...
 function startEventStream(message) {
    ...
    // 选项值，
    // 组装01；http://101.126.92.213:8090/api/v1/ollama/generate_stream?message=Hello&model=deepseek-r1:1.5b
    // 组装02；http://101.126.92.213:8090/api/v1/openai/generate_stream?message=Hello&model=gpt-4o
    ...
    if (ragTag) {
        url = `http://101.126.92.213:8090/api/v1/${aiModelValue}/generate_stream_rag?message=${encodeURIComponent(message)}&ragTag=${encodeURIComponent(ragTag)}&model=${encodeURIComponent(aiModelModel)}`;
    } else {
        url = `http://101.126.92.213:8090/api/v1/${aiModelValue}/generate_stream?message=${encodeURIComponent(message)}&model=${encodeURIComponent(aiModelModel)}`;
    }
